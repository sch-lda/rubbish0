...
class Program
{
    ...
    {
        string luaFileName = "C:/Users/SongCH/AppData/Roaming/YimMenu/scripts/sch.lua"; // 替换为你的Lua文件路径
        string outputFileName = "C:/Users/SongCH/AppData/Roaming/YimMenu/scripts/sch.lua.txt"; // 替换为输出文件名
        ...
        try
        {
            ...
            using (StreamWriter writer = new StreamWriter(outputFileName))
            {
                foreach (Match match in matches)
                {
                    ...
                    if (ContainsAnyKeyword(line, keywords) && ContainsChineseCharacters(match.Groups[1].Value) && !IsWhiteSpaceOrPercent(match.Groups[1].Value))
                    {
